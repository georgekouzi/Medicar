...
class AlarmsAdapter(
    activity: SimpleActivity, var alarms: ArrayList<com.simplemobiletools.clock.models.Alarm>, val toggleAlarmInterface: ToggleAlarmInterface,
    ...
    private fun getSelectedItems() = alarms.filter { selectedKeys.contains(it.id) } as ArrayList<com.simplemobiletools.clock.models.Alarm>

    private fun setupView(view: View, alarm: com.simplemobiletools.clock.models.Alarm) {
        val isSelected = selectedKeys.contains(alarm.id)
        view.apply {
            alarm_frame.isSelected = isSelected
            alarm_time.text = "Next intake: ${activity.getFormattedTime(alarm.timeInMinutes * 60, false, true)}"
            pill_date.text = "(${alarm.date})"

            alarm_days.text = "Days : ${activity.getAlarmSelectedDaysString(alarm.days)}"

            alarm_label.text = "Name : ${alarm.pillName}"
            pill_description.text = "Description : ${alarm.pillDescription}"

            profile_image.setImageURI(Uri.parse(alarm.image))


          alarm_label.beVisibleIf(alarm.pillName.isNotEmpty())
